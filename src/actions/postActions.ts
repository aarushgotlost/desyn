...
export async function getPostLikeStatus(
  ...
  if (!userId || !postId) {
    // Try to get post likes even if user is not logged in
    const postRef = doc(db, 'posts', postId);
    const postSnap = await getDoc(postRef);
  ...
  try {
    ...
    return { isLiked: false, likesCount: 0 }; // Default to not liked on error
  }
}
...
export async function addCommentToPost(
  ...
  try {
    ...
    const postSnap = await getDoc(postRef); // Get post data to find communityId for revalidation
    if (postSnap.exists()) {
        const postData = postSnap.data() as Post;
        await revalidatePostPaths(postId, postData.communityId);
    } else {
        await revalidatePostPaths(postId); // Fallback if post data not found immediately
    }
    

    return { success: true, message: 'Comment added!', commentId: newCommentRef.id };
  } catch (error: any) {
