...
export function LikeButton({ postId, initialLikesCount, initialIsLiked, size = "sm", showText = true }: LikeButtonProps) {
  const { user, userProfile, loading: authLoading } = useAuth(); // Added userProfile
  ...
  const handleLike = async () => {
    if (!user || !userProfile) { // Ensure userProfile is available for notification
    ...
    startTransition(async () => {
      ...
      // Pass liker's profile for notification creation
      const likerInfo = {
        uid: userProfile.uid,
      ...
      const result = await togglePostLike(postId, user.uid, likerInfo);
      if (result.success) {
